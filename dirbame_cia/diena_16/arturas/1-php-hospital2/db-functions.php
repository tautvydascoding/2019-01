...
function createPatient($vardas, $pavarde, $doctors_id) {
  $manoSQL = "INSERT INTO patients VALUES (NULL, '$vardas', '$pavarde', '$doctors_id')";
  $SQLquery = mysqli_query(getPrisijungimas(), $manoSQL);
  if (!$SQLquery) {
    echo "Sukurti paciento $vardas $pavarde nepavyko. ".mysqli_error(getPrisijungimas());
  } else {
    echo "Pacientas $vardas $pavarde sukurtas!";
  }
}
...
function deletePatient($nr) {
  $manoSQL = "DELETE FROM patients WHERE id = $nr LIMIT 1";
  $SQLquery = mysqli_query(getPrisijungimas(), $manoSQL);
  if (!$SQLquery) {
    echo "Paciento Nr. $nr ištrinti nepavyko! ".mysqli_error(getPrisijungimas());
  } else {
    echo "Pacientas Nr. $nr ištrintas!";
  }
}
...
function getPatients($kiekis = 9999) {
  $manoSQL = "SELECT * FROM patients LIMIT $kiekis";
  $rezultatai = mysqli_query(getPrisijungimas(), $manoSQL);
  return $rezultatai;
}
